...
export const load = async ({ fetch, params }) => {
    ...
    const client = axios.create({
        baseURL:
            `https://api.stormglass.io/v2/weather`,
        headers: { Authorization: STORMGLASS_API_KEY }
    });

    console.log(`Querying stormglassio for ${response.name}, ${response.latitude}, ${response.longitude}`)
    const endDateTime = DateTime.now().plus({days: 4})
    const result = (await client.get('/point', { params: { lat: response.latitude, lng: response.longitude, params: "swellHeight,windSpeed", end: endDateTime } })) as AxiosResponse;
